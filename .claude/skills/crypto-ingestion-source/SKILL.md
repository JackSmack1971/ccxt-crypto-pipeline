...
description: Implements, modifies, and debugs a repository's external crypto-data ingestion boundary, including CCXT CEX adapters; DexScreener, GeckoTerminal, and DefiLlama; EVM explorer or risk-screen clients; and Helius/Solana ingestion. Use for provider or endpoint selection, auth/paywalls/rate limits, response normalization, canonical IDs, idempotent writes, and ingestion-run verification. Do not use for storage-schema migrations, scheduler/orchestration wiring, downstream analysis/scoring/trading, backtests, or generic API research that does not change ingestion behavior.
---
...
1. **Claude/repository instructions first.** Read the applicable `CLAUDE.md` chain and any repository-declared cross-tool policy files before inspecting implementation. Then inspect repository schemas/models, `config/`, ingestion interfaces, tests, and run logging before editing. Do not invent tables, fields, config keys, or helper names from this skill.
