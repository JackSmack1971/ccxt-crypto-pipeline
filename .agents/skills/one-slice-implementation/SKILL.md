...
description: >-
  Advance an existing Git repository by exactly one smallest coherent, independently
  verifiable implementation slice, verify it, stage only attributable changes, and stop
  for review. Use only when explicitly invoked for a stage-only implementation pass;
  not for planning, auditing, broad feature completion, refactors, commits, merges,
  pushes, pull requests, deployment, or multiple independent slices.
metadata:
  version: "1.2.0"
---
...
Read every applicable repository instruction in scope, including `AGENTS.md`, `AGENTS.override.md`, and repository-defined policy or precedence rules.
