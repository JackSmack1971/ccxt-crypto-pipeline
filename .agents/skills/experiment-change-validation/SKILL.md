...
description: >-
  Validate repository changes that can alter research or experiment methodology, including
  ...
  identity. Map the affected methodological contracts, run the narrowest sufficient focused
  ...
  policy justifies it. Not for ordinary ingestion/storage/reporting edits that cannot change
  research semantics, generic code review, or implementing the change itself.
metadata:
  version: "0.1.0"
---
...
Read the applicable instruction chain first, especially root `AGENTS.md` and `analysis/AGENTS.md`, plus any deeper scoped instructions. Treat current code, tests, schemas/configuration, and accepted specifications as stronger evidence than stale plans.
...
After focused executable checks pass, invoke the configured Codex subagent **`experiment_integrity_reviewer`** with the handoff contract in [Reviewer handoff](references/REVIEWER_HANDOFF.md).
