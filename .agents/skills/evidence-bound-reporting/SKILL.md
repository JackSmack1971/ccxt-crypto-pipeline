...
description: >-
  Use when implementing, modifying, or debugging this repository's Phase 4
  evidence-bound reporting boundary: approved-research handoffs, factual
  claim/derivation validation, chart specifications or transformations,
  deterministic rendering, accessibility/provenance checks, or review-package
  generation. Activate when reporting output must remain provably derived from
  immutable approved local research. Do not use for Phase 3
  cohort/feature/label/statistical methodology, Phase 2 backtest semantics,
  ingestion/provider work, storage-schema migrations, free-form editorial
  writing, or external publication/distribution.
---
...
1. Read the active user goal, every applicable `AGENTS.md` / `AGENTS.override.md`, `ROADMAP.md`, the active Phase 4 plan/decision document, and the relevant reporting code/tests before editing.
